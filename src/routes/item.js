...
router
  ...
  .post(passport.authenticate('jwt', { session: false }), function(req, res) {
    if (
      ...
      collection.update(query, newData, { upsert: true }, function(
        err,
        result
      ) {
        if (err) {
...
router
  ...
  .post(passport.authenticate('jwt', { session: false }), function(req, res) {
    if (!req.body.itemname || !req.body.ans) {
      res.json({
        success: false,
        message: 'Please specify object name and its status',
      });
    } else {
      var today = new Date();
      var dd = today.getDate();
      ...
      var collection = db.collection('items');
      collection.update(
        { shopkeeper: user, name: req.body.itemname },
        { $set: { status: req.body.ans, lastUpdate: dateStr } },
